...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `discounts_BEFORE_UPDATE` BEFORE UPDATE ON `discounts` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ALLOW_INVALID_DATES,ERROR_FOR_DIVISION_BY_ZERO,TRADITIONAL,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `discounts_BEFORE_DELETE` BEFORE DELETE ON `discounts` FOR EACH ROW BEGIN
	IF((SELECT count(*) FROM customers WHERE OLD.id = discounts_id) > 0) THEN
		SIGNAL SQLSTATE '45000';
	END IF;
END */;;
DELIMITER ;
